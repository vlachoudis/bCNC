...
class Tool(Plugin):
	...
	def __init__(self, master):
		Plugin.__init__(self, master, "Sinuous")
		self.icon  = "gear"
		self.group = "CAM"
		self.variables = [
			("name",      "db",    "", _("Name")),
...
#			("x_precision","mm",  5.0, _("X Precision")),
			...
			("holes"    ,    "bool" ,    True, _("Holes")),
			("depth","float",  0, _("Holes depth")),
		]
...
#		xpoint=alpha
		...
		block.append("G0 x" + str(xpoint)+" y "+str(r1-base))
...
#		xpoint=final_alpha
		...
		block.append("G1 x" + str(xpoint)+" y "+str(r1-base))
		block.append("G1 x 0.0" +" y "+str(r1-base))
...
#			i=i+1
	...
	def holes(self, r1, depth):
		...
		d=r1*pi/2
		while i<(large):
			block.append("(i "+ str(i)+" )")
#			block.append("G0 X "+ str(r1*pi/2.0) + " y "+str(r1-base/2.0))
			block.append("G0 X "+ str(d0+4*i*d) + " y "+str(r1-base/2.0))
			block.append("F 100 G1 Z "+str(depth))
			block.append(CNC.zsafe()) 			#<<< Move rapid Z axis to the safe height in Stock Material
			i=i+0.5
		return block
	# ----------------------------------------------------------------------
...
#		self.finish_blocks(app, blocks)
		...
		block = Block("Sinuous R1 "+str(r1)+" R2 "+str(r2)+" angle init "+str(alpha))
#		block.append(CNC.zsafe()) 			#<<< Move rapid Z axis to the safe height in Stock Material
		block.extend(self.rack(r1,r2))

		depth=self["depth"]
		holes=self["holes"]
		holesblock=Block("Holes")
		holesblock.extend(self.holes(r1,depth))

		blocks.append(block)
		if holes:
			blocks.append(holesblock)
		active = app.activeBlock()
