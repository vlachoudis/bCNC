...
VIEW_ISO3 = 5
VIEW_ZX   = 6
VIEWS     = ["X-Y", "X-Z", "Y-Z", "ISO1", "ISO2", "ISO3", "Z-X"]
...
#		self.tag_lower(self._workarea)
	...
	def initPosition(self):
		...
		if self.view == VIEW_XY:
			...
			gy = gr//2
			gh = 3*gr
			if self.view in (VIEW_ZX, VIEW_XZ, VIEW_YZ):
				self._gantry1 = None
	...
	def drawOrient(self, event=None):
		...
		if self.view in (VIEW_ZX, VIEW_XZ, VIEW_YZ): return
...
#		b.pack(side=LEFT)
	...
	# ----------------------------------------------------------------------
	def viewZX(self, event=None):
		self.view.set(VIEWS[VIEW_ZX])
