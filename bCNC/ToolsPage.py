...
class _Base:
	...
	def populate(self):
		...
		if len(varhelp) > 0:
			for line in varhelp.splitlines():
				if len(line) > 0 and line[0] == '#' and line[1:] in list(Utils.images.keys()):
	...
	def save(self):
		...
		if self.listdb:
			for name,lst in list(self.listdb.items()):
...
class Tools:
	...
	# ----------------------------------------------------------------------
	def pluginList(self):
		plugins = [x for x in list(self.tools.values()) if x.plugin]
	...
	# ----------------------------------------------------------------------
	def names(self):
		lst = [x.name for x in list(self.tools.values())]
		lst.sort()
		return lst
...
#		self.addWidget(b)
	...
	def languageChange(self):
		...
		# find translation
		for a,b in list(Utils.LANGUAGES.items()):
			if b == lang:
