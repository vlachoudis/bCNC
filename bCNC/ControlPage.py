...
class ControlFrame(CNCRibbon.PageLabelFrame):
	def __init__(self, master, app):
		...
		self.step.fill(list(map(float, Utils.config.get("Control","steplist").split())))
		...
		try:
			...
			zsl = [_NOZSTEP]
			zsl.extend(list(map(float, Utils.config.get("Control","zsteplist").split())))
			self.zstep.fill(zsl)
...
class StateFrame(CNCRibbon.PageExLabelFrame):
	def __init__(self, master, app):
		...
		for k,v in list(DISTANCE_MODE.items()):
		...
		for k,v in list(UNITS.items()): self.gstate[k] = (self.units, v)
		...
		for k,v in list(PLANE.items()): self.gstate[k] = (self.plane, v)
		...
		for k,v in list(FEED_MODE.items()): self.gstate[k] = (self.feedMode, v)
