	...
	def load(self, filename=None):
		...
		def read(f):
			while True:
				...
				if len(line)==0: raise
				line = line.strip()
				if line: return list(map(float, line.split()))
...
#		print "r=",r
	...
	def load(self, filename=None):
		...
		f = open(self.filename,"r")
		for line in f:
			self.add(*list(map(float, line.split())))
		f.close()
...
class CNC:
	...
	def compile(program):
		...
		for j,line in enumerate(program):
			...
			if isinstance(cmds,str) or isinstance(cmds,str):
...
#		self.probe.init()
	...
	def importDXF(self, filename):
		...
		undoinfo = []
		for name,layer in list(dxf.layers.items()):
	...
	def autolevelBlock(self, block):
		...
		for line in block:
			...
			if cmds is None:
				new.append(line)
				continue
			elif isinstance(cmds,str) or isinstance(cmds,str):
				cmds = CNC.breakLine(cmds)
			else:
				new.append(line)
				continue
