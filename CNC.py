...
#				undoinfo.extend(self.addBlockOperationUndo(bid,"CCW"))
	...
	def saveSVG(self, filename):
		...
		svg.write('<svg viewBox="%s %s %s %s">\n'%(minx*scale, -maxy*scale, (maxx-minx)*scale, (maxy-miny)*scale))
		...
		for block in self.blocks:
			...
			for line in block:
				...
				if self.cnc.gcode == 1:	# line
					#dxf.line(self.cnc.x, self.cnc.y, self.cnc.xval, self.cnc.yval, name)
					svg.write('\t<path d="M %s %s L %s %s" stroke="%s" stroke-width="%s" fill="none" />\n'%(self.cnc.x*scale, -self.cnc.y*scale, self.cnc.xval*scale, -self.cnc.yval*scale, color, width))
				elif self.cnc.gcode in (2,3):	# arc
					#FIXME: does not work for circles, only arcs that are not all the way round
					xc,yc = self.cnc.motionCenter()
					sphi = math.atan2(-self.cnc.y-yc,    self.cnc.x-xc)
					ephi = math.atan2(-self.cnc.yval-yc, self.cnc.xval-xc)
					arcSweep = ephi - sphi
					...
					if self.cnc.gcode==2:
						if ephi<=sphi+1e-10: ephi += 2.0*math.pi
						#dxf.arc(xc,yc,self.cnc.rval, math.degrees(ephi), math.degrees(sphi),name)
						svg.write('\t<path d="M %s %s A %s %s %s %s %s %s %s" stroke="%s" stroke-width="%s" fill="none" />\n'%(self.cnc.x*scale, -self.cnc.y*scale, self.cnc.rval*scale, self.cnc.rval*scale, 0, arcSweep, 1, self.cnc.xval*scale, -self.cnc.yval*scale, color, width))
					else:
						if ephi<=sphi+1e-10: ephi += 2.0*math.pi
						#dxf.arc(xc,yc,self.cnc.rval, math.degrees(sphi), math.degrees(ephi),name)
						svg.write('\t<path d="M %s %s A %s %s %s %s %s %s %s" stroke="%s" stroke-width="%s" fill="none" />\n'%(self.cnc.x*scale, -self.cnc.y*scale, self.cnc.rval*scale, self.cnc.rval*scale, 0, arcSweep, 0, self.cnc.xval*scale, -self.cnc.yval*scale, color, width))
				self.cnc.motionEnd()
		#dxf.writeEOF()
		svg.write('</svg>\n')
		svg.close()
